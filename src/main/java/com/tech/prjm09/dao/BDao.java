...
import com.tech.prjm09.utill.DBCon;
...
public class BDao {
	...
	public BDto contentView(String sbid) {
		...
		try {
			conn=DBCon.getConnection();
			String query="SELECT bid,bname,btitle,bcontent,bdate,bhit,bgroup,bstep,bindent from replyboard WHERE bid=?";
			pstmt=conn.prepareStatement(query);
			...
			try {
				if(rs!=null) {rs.close();} if(pstmt!=null) {pstmt.close();} if(conn!=null) {conn.close();}} catch (Exception e2) {}}			
		return dto;
	}
